...
describe("test Select component", () => {
  it("should render the correct Select component", () => {
    ...
    const firstItem = getByText("hello");
  ...
  it("Select in multiple mode should works fine", () => {
    ...
    // add tags
    expect(container.querySelectorAll(".quib-tag").length).toEqual(1);
    //remove placeholder
    expect(inputEle.placeholder).toEqual("");
    // click 2nd item
    fireEvent.click(secondItem);
    expect(multipleProps.onChange).toHaveBeenLastCalledWith("id2", ["id1", "id2"]);
    expect(container.querySelectorAll(".quib-tag").length).toEqual(2);
    //reclick 2nd item
    fireEvent.click(secondItem);
    // remove acitve class
    ...
    //remove all tags
    expect(container.querySelectorAll(".quib-tag").length).toEqual(0);
